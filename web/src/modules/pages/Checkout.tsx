...
export default function Checkout() {
  ...
  const payVodafone = async () => {
    setTouched({ fullName: true, phone: true, address: true, city: true, postalCode: true })
    if (!validateShipping()) { show('Please complete shipping info', 'error'); return }
    if (!shipping.phone) { show('Phone is required for Vodafone Cash', 'error'); return }
    setIsSubmitting(true)
    try {
      // Create backend order from cart
      const order = await api<{ id: number; number: string }>(`/orders/create`, { method: 'POST', body: JSON.stringify({}) })
      // Initiate Paymob wallet payment
      const init = await api<{ paymentToken: string }>(`/payments/wallet/initiate`, { method: 'POST', body: JSON.stringify({ orderId: (order as any).id || (order as any).order?.id || 0, phone: shipping.phone }) })
      show('Vodafone Cash initiated. Complete payment on your phone.', 'success')
      // Display token for debugging or deep-link handoff
      console.log('Paymob wallet token:', init.paymentToken)
      navigate('/order-confirmation')
    } catch (e) {
      show('Vodafone Cash initiation failed', 'error')
    } finally {
      setIsSubmitting(false)
    }
  }
              ...
              <div className="glass border border-white/10 rounded-xl p-6">
                ...
                <div className="space-y-4">
                  ...
                  <motion.button
                    ...
                    whileTap={{ scale: 0.98 }}
                    onClick={method === 'vodafone' ? payVodafone : placeManualOrder}
                  ...
                  >
                    {isSubmitting ? (
                      ...
                      method === 'vodafone' ? 'Pay with Vodafone Cash' : 'Place Order'
                    )}
                  </motion.button>
